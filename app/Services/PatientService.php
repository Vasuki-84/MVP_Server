...
class PatientService {
    ...
    // Resolve patient.id from the logged-in patient user, then return their rx
    public static function getForLoggedInPatient(int $userId) {
        $db = getDB();
        $stmt = $db->prepare(
            "SELECT id FROM patients WHERE user_id = ? AND deleted_at IS NULL"
        );
        $stmt->execute([$userId]);
        $row = $stmt->fetch();
        if (!$row) {
        ...
        return self::getByPatient((int) $row['id']);
    }

    // Guard for GET /prescriptions/{id} when called by a patient
    public static function assertOwnedByUser(array $rx, int $userId): void {
        $db = getDB();
        $stmt = $db->prepare(
            "SELECT id FROM patients WHERE id = ? AND user_id = ? AND deleted_at IS NULL"
        );
        $stmt->execute([$rx['patient_id'], $userId]);
        if (!$stmt->fetch()) {
