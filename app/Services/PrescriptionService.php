...
class PrescriptionService {
    ...
    public static function getForLoggedInPatient($userId) {
        $db   = getDB();
        $stmt = $db->prepare("
            SELECT p.*, u.name AS doctor_name, pt.id AS patient_record_id
            FROM prescriptions p 
    ...
    public static function assertOwnedByUser($rx, $userId) {
        $db = getDB();
        // $rx contains `patient_id` which is the ID from the patients table.
        // We find the user_id for that patient.
        $stmt = $db->prepare("SELECT user_id FROM patients WHERE id = ?");
        $stmt->execute([$rx['patient_id']]);
        $patient = $stmt->fetch();
        if (!$patient || (int)$patient['user_id'] !== $userId) {
            Response::error('Unauthorized to view this prescription', 403);
        }
    }

    // Scoped list for a logged-in doctor — only their own prescriptions
    public static function getForDoctor($doctorUserId) {
        $db   = getDB();
        $stmt = $db->prepare("
            SELECT p.*, u.name AS doctor_name, pt.id AS patient_record_id
            FROM prescriptions p
    ...
    // Ensure a prescription was created by the given doctor (user_id)
    public static function assertOwnedByDoctor($rx, $doctorUserId) {
        if ((int)$rx['doctor_id'] !== $doctorUserId) {
            Response::error('Unauthorized: you can only manage your own prescriptions', 403);
        }
    }
